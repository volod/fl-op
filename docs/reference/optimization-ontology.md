...
|---|---|---|
| Drone logistics | `contracts/domains/drone_logistics/` (default: data generator + solver wiring) | UGVs/UAVs/payload modules/operators -> asset roles; logistics hubs (with battery inventory plus `chargingPowerKw`/`chargingSlots` station capacity), delivery locations, and restricted zones -> location; delivery orders -> task variants with `alternativeGroupRef`; weather -> forecast; road/air links -> travel-link with `networkMode`; prices -> cost-rate. Post-solve passes derive 4D airspace deconfliction (altitude corridors + deadline-bounded temporal holds) and hub charging-queue schedules with turnaround readiness into `drone_logistics_kpis`. |
